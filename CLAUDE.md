...
- **Game data** is in `site/src/app/data/projects.ts` — a plain TypeScript array of game objects. No database, no API. FAQ copy lives in `site/src/app/data/faqs.ts` so the FAQ page can render it *and* emit `FAQPage` structured data server-side.
- **SEO helpers** are in `site/src/app/lib/seo.tsx`: `BASE_URL`, the `JsonLd` component, `pageOg()`, and the schema builders. A page-level `openGraph` **replaces** the layout's, so always build it with `pageOg()` or the page unfurls with no image.
- **`/llms.txt`** is generated by `site/src/app/llms.txt/route.ts` from `projects.ts` + `faqs.ts`, so it never goes stale.
- **Static assets** go in `site/public/assets/`. Reference them as `/assets/<filename>` or via the `asset()` helper in SitePages.tsx.

## Cloudflare / performance constraints
- Build with `npm run cf:build` (build **plus** `populateCache local`). Plain `opennextjs-cloudflare build` leaves the prerendered pages out of `.open-next/assets`, so every request boots the Next.js server.
- Do **not** set `enableCacheInterception` in `open-next.config.ts`. It mishandles RSC prefetch (`?_rsc=`) responses and puts the App Router into an endless prefetch retry loop (~94 req/s per open tab), which burns the Workers request quota.
- Never use a CSS `background-image` for a photo — it bypasses the image optimizer and ships the full-size source. Use a `fill` `<Image>` in a `position: relative; overflow: hidden` parent.
