...
function EngineIcon({ engine, size = 24 }: { engine: string; size?: number }) {
  ...
  if (!e) return null;
  if (e.icon) return <div className="engine-logo" style={{ width: size, height: size }}><img src={e.icon} alt={e.label} title={e.label} /></div>;
...
function PhotoHero({ kicker, title, body }: { kicker: string; title: string; body?: string }) {
  return <section className="photo-hero"><div className="photo-hero-bg" style={{ backgroundImage: `url(${asset("showcase.jpg")})` }} /><div className="container"><div className="section-kicker">{kicker}</div><h1>{title}</h1>{body && <p>{body}</p>}</div></section>;
}
...
export function FaqPage() {
  ...
  return <PageShell><main className="page-enter"><section className="faq-hero"><div className="faq-hero-bg" style={{ backgroundImage: `url(${asset("showcase.jpg")})` }} /><div className="faq-hero-inner"><h1>Frequently Asked Questions</h1><p>Have a question about our club? Here are the most frequent ones - if yours isn't here, ping us on Discord or via the contact page.</p></div></section><section className="faq-card-band"><div className="faq-cards">{FAQ_CARDS.map((c) => <div key={c.num} className="faq-card"><div className="faq-card-num">{c.num}</div><div className="faq-card-body"><h4>{c.q}</h4><p>{c.a}</p></div></div>)}</div></section><section className="section faq-section"><div className="faq-list"><div className="section-kicker">Everything else</div><h2 className="section-title dark">The full list.</h2>{FAQS.map((item, i) => <div key={item.q} className={`faq-item ${open === i ? "open" : ""}`}><button className="faq-q" onClick={() => setOpen(open === i ? -1 : i)}><span>{item.q}</span><span className="faq-icon" /></button>{open === i && <div className="faq-a">{item.a}</div>}</div>)}</div><div className="faq-contact"><p>Didn't find your answer?</p><a href="https://discord.gg/ZZU5xQbv8K" className="btn btn-discord"><Icon name="discord" size={18} /> Ask in Discord</a><Link href="/contact" className="btn btn-ghost">Contact a board member <Icon name="arrow-right" size={16} /></Link></div></section></main></PageShell>;
}
