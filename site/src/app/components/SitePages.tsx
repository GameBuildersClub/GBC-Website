...
function Nav() {
  ...
  return <nav className="nav"><div className="nav-inner"><Link href="/" className="nav-brand"><Image src={asset("logo.svg")} alt="GBC" width={40} height={40} /><span>Game Builders Club</span></Link><div className="nav-links">{links.map((l) => <Link key={l.href} href={l.href} className={`nav-link ${pathname === l.href ? "active" : ""}`}>{l.label}</Link>)}</div><div className="nav-socials"><a href="https://www.instagram.com/gamebuildersclub/" className="nav-social" aria-label="Instagram" target="_blank" rel="noreferrer"><Icon name="instagram" size={22} /></a><a href="https://discord.gg/ZZU5xQbv8K" className="nav-social" aria-label="Discord" target="_blank" rel="noreferrer"><Icon name="discord" size={22} /></a><button type="button" className="nav-hamburger" aria-label="Open menu" onClick={() => setMenuOpen(true)}><Icon name="menu" size={22} /></button></div></div>{menuOpen && <div className="nav-mobile-overlay" onClick={() => setMenuOpen(false)}><div className="nav-mobile-menu" onClick={(e) => e.stopPropagation()}><div className="nav-mobile-head"><Link href="/" className="nav-brand" onClick={() => setMenuOpen(false)}><Image src={asset("logo.svg")} alt="GBC" width={40} height={40} /><span>Game Builders Club</span></Link><button type="button" className="nav-mobile-close" aria-label="Close menu" onClick={() => setMenuOpen(false)}><Icon name="x" size={20} /></button></div><div className="nav-mobile-links">{links.map((l) => <Link key={l.href} href={l.href} className={`nav-mobile-link ${pathname === l.href ? "active" : ""}`} onClick={() => setMenuOpen(false)}>{l.label}</Link>)}</div><div className="nav-mobile-socials"><a href="https://www.instagram.com/gamebuildersclub/" aria-label="Instagram" target="_blank" rel="noreferrer"><Icon name="instagram" size={22} /></a><a href="https://discord.gg/ZZU5xQbv8K" aria-label="Discord" target="_blank" rel="noreferrer"><Icon name="discord" size={22} /></a></div></div></div>}</nav>;
}

function Footer() {
  return <footer className="footer"><div className="footer-inner"><div><div className="footer-brand-row"><Image src={asset("logo.svg")} alt="GBC" width={40} height={40} /><div><strong>Game Builders Club</strong><br /><span>University of Georgia · est. 2021</span></div></div><p>A student-run club for making games at UGA. Designers, programmers, artists, writers and musicians, all welcome.</p></div><FooterLinks title="Site" links={[["Home", "/"], ["Games", "/games"], ["About Us", "/about"], ["How it Works", "/how-it-works"], ["FAQ", "/frequently-asked-questions"], ["Contact", "/contact"]]} /><FooterLinks title="Find Us" links={[["Discord", "https://discord.gg/ZZU5xQbv8K"], ["Instagram", "https://www.instagram.com/gamebuildersclub/"]]} /></div><div className="footer-bottom"><span>© 2026 Game Builders Club · University of Georgia · Built by members, for members.</span><div className="footer-socials"><a href="https://discord.gg/ZZU5xQbv8K" aria-label="Discord" target="_blank" rel="noreferrer"><Icon name="discord" size={16} /></a><a href="https://www.instagram.com/gamebuildersclub/" aria-label="Instagram" target="_blank" rel="noreferrer"><Icon name="instagram" size={16} /></a></div></div></footer>;
}

function FooterLinks({ title, links }: { title: string; links: string[][] }) {
  return <div><h5>{title}</h5><div className="footer-links">{links.map(([label, href]) => href.startsWith("/") ? <Link key={label} href={href}>{label}</Link> : <a key={label} href={href} target="_blank" rel="noreferrer">{label}</a>)}</div></div>;
}
